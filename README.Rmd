...
The schematic figure below summarises the steps in the data analysis workflow for the baseline survey:
...
cat(
  ...
  targets::tar_mermaid(
    targets_only = TRUE, 
    names = c(dplyr::starts_with("baseline")),
    shortcut = TRUE,
    exclude = dplyr::contains(c("strata", "study_group", "table"))
  ),
  "```",
  sep = "\n"
)
...
This repository uses `git-crypt` to enable transparent encryption and decryption of the `.env` file and the `auth/` directory.
